...
!C    =================================================================
      subroutine bbr_particle(pmx,pmy,pmz,pmk1,pmk2,pma, pmrwa,pmrwf,pmrwd, fpm1,fpm2,fpm3,fpm4,fpm5,fpm6)
!C    
!C    This version was modified by FS in order to compile into a Python shared library.
!C    This subroutine takes the parameters that are in bbr.f90 taken in through param.txt
!C
!C    Compile using:    
!C    f2py -c -m apexd_particle bbr_fs.f90
!C    =================================================================
      use mod_variable
      implicit none
      ...
      double precision :: tini,dt,Tsq,ddt,xu,xv, kini,xdr,ydr,zdr,kini1,kini2,xpr,ypr,zpr,rsa,rPsi,rPsipr
      double precision :: xv1,xv2,xv1int,kang,bmb1,bmb2
      double precision :: xi,yi,zi,Ex,Ey,Ez,Bx,By,Bz

      integer :: i,iini,imbk,Nr,itf,ifile,ifile2,imbk2,imbk2t,iclmap,iclbp,iclbp2,imu,ntr,ntrpr
      integer :: its,ikene,ipm,ipm1,ipm2,ipmt
      ...
      double precision,dimension(0:1001) :: pmx,pmy,pmz,pmk1,pmk2,pma
!f2py intent(in) :: pmx,pmy,pmz,pmk1,pmk2,pma
      double precision,dimension(0:1001) :: pmk,pmrx,pmry,pmrz
      double precision,dimension(0:1001) :: pmrwa,pmrwf,pmrwd
!f2py intent(in) :: pmrwa,pmrwf,pmrwd      
...
!f2py intent(in) :: fpm1,fpm2,fpm3,fpm4,fpm5,fpm6
      character filename*128,filename0*128,filenameJ*128,filenamemu*128
      integer dtm(8)
      character(len=10) sys_time(3)
      integer,parameter :: max_line_len1 = 100
      character(max_line_len1) cf2
...
!C    ##### line numbers in param.txt to be used in the calculation #### パラメータファイルの行数指定
      ! fix the following to (1,1) to always use 1 particle at a time
...
!C    ##### save additional files 追加的な計算も保存 ####
      icmp=1; ! 1: 
!C    ##### save files for statistics 密度計算用のファイルを保存するか ####
...
!C    ##### E is given by linear interpolation 電場データを補完して使用 ####
      iexe=1; ! 0:given as constant values  1: use external E field file
...
!      write(6,*) u(1),u(2),u(3)
      ...
      if (ikene == 2) then
!C    parallel and perpendicular K(eV) 縦と横のK(eV)で与える
...
!     Jを計算するために，磁力線に沿った積分を行う．赤道面に帰るたびに出力．
      xv1int = xv1int + xv1*xv1*dt  !\int \vec{v} \cdot \vec{dl}
