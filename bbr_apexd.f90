...
!C     2. copy e3d_hd.txt(grid information) and e3d*.txt into the same folder as bbr.f90.
!C     3. compile and run bbr.90 with param.txt, which can be made by param.f90.
...
!C    ##### physical parameters, start ####
      ...
      double precision,dimension(1:205,1:205,1:205,1:4) :: Exm,Eym,Ezm ! electric fields (unit field by electrodes) max9 電場ファイル (電極による単位電場) max9
...
!C    =================================================================
      subroutine bbr_particle(pmx,pmy,pmz,pmk1,pmk2,pma, pmrwa,pmrwf,pmrwd, fpm1,fpm2,fpm3,fpm4,fpm5,fpm6, ipmt)
!      program bbr
!      , stt,stx,sty,stz,str,stgx,stgy,stgz,stgr,stK,stKpr,stKpp,stps1,stps2,stmu)
!      program bbr 
!    
!     This version was modified by FS in order to compile into a Python shared library.
!     This subroutine takes the parameters that are in bbr.f90 taken in through param.txt
!
!     Compile using:    
!     f2py -c -m bbr_particle bbr_fs.f90
!     or to use a signature file:
!        f2py bbr_particle.f90 -m bbr_particle -h bbr_particle.pyf only: bbr_particle --overwrite-signature
!        and f2py -c bbr_particle.pyf bbr_particle.f90
!        
!     HS output files:
!     orb****rk.txt:
!     1:x 2:y 3:z 4:r 5:vx 6:vy 7:vz 8:time
!     9:Kttl 10:Kpara 11:Kperp 12:Bx 13:By 14:Bz 15:B 16:Ex 17:Ey 18:Ez 19:Psi
!     20:xg.c. 21:yg.c. 22:zg.c. 23:rg.c. 24:Psi_g.g. 25:mu 26:t.rotation
!     splot "orb0001bbr.txt" w l
!
!     Indices ---> mu:24 (averaged over cyclotron orbit),J:22 (averaged over bounce motion)
!
!     orb_a0rk.txt,orb_a1rk.txt,orb_a2rk.txt: averatge, average+sigma, average-sigma
!     1:x 2:y 3:z 4:r 5:gx 6:gy 7:gz 8:gr 9:K 10:Kpr 11:Kpp 12:Ps 13:gPs 14:mu 15:time  # add t.rotation
!
!C    =================================================================
      use mod_variable
      implicit none
      ...
      double precision :: tini,t,dt,Tsq,ddt,tt,xu,xv, kini,xdr,ydr,zdr,kini1,kini2,xpr,ypr,zpr,rsa,rPsi,rPsipr
      double precision :: xv1,xv2,xv1int,kang,bmb1,bmb2,bmb3,ken,tmax,tsdiv,dt2
      double precision :: xi,yi,zi,vxi,vyi,vzi, Ex,Ey,Ez,Bx,By,Bz
      integer :: i,iini,imbk,Nr,ir,itf,ifile,ifile2,imbk2,imbk2t,iclmap,iclbp,iclbp2,imu,ntr,ntrpr
      integer :: its,ikene,iparam,ipm,ipm1,ipm2,ipmt
      double precision,dimension(1:6) :: Xr,k1,k2,k3,k4,ywork
      double precision :: Hr,Tr, kene,xgm,xbt,rnd, Btt,Psi,ken1,ken2,Psi1,Psi2,xg1,yg1,zg1,xgpr1,ygpr1,zgpr1
      double precision :: rpst,rsgm,pinix,piniy,piniz,gr,gr1,xmu
      integer(8) :: it,iend,iend2
      integer t1, t2, t_rate, t_max, diff,iex1,iex2,iex3,iex4,icmp,ihit,izero,ieout,ieoutc,iden,iif,irsa,irsapr
      double precision,dimension(1:100005) :: elik2,elie2
      double precision,dimension(1:1001) :: pmx,pmy,pmz,pmk1,pmk2,pma,pmk,pmrx,pmry,pmrz
      double precision,dimension(1:1001) :: pmrwa,pmrwf,pmrwd
      double precision,dimension(1:1001) :: fpm1,fpm2,fpm3,fpm4,fpm5,fpm6 ! for param.txt can be used as vfp1,vfp2,vfp3,vfp4,vfp5,vfp6 as global parameters
...
!            Bxm(iex1,iex2,iex3),Bym(iex1,iex2,iex3),Bzm(iex1,iex2,iex3);
      ...
      open(17, file=cf2) !C electrode 電極1
      do iex3=1,nz+1
        do iex2=1,ny+1
          do iex1=1,nx+1
...
!!      kini=1.7588200236042485d1; xdr=0.0d0; ydr=1.0d0; zdr=1.0d0; ! -> rL=0.1mm
      ...
      dtm(1),".",dtm(2),".",dtm(3)," ",dtm(5),":",dtm(6),".",dtm(7), " # Boris relativistic #"
...
!      write(6,*) xv1 * B(3) / bmb1, xv2 * (-B(1) / bmb2 * dcos(kang)), -xv2 *  B(2)*B(3) / (bmb1*bmb2) * dsin(kang) 
      ...
      x(1)=0.0d0; x(2)=0.0d0; x(3)=0.0d0; v(1)=0.0d0; v(2)=0.0d0; v(3)=0.0d0; u(1)=0.0d0; u(2)=0.0d0; u(3)=0.0d0; 
...
!C    'XU (four velocity), XV (real velocity)'
      ...
      xbt = xv/xc; xgm = 1.0d0/dsqrt(1.0d0-xbt*xbt) ! this is v^n-1/2，これはv^n-1/2，
...
!      endif
      ...
      if (ifile == 1) then ! orbit output files 軌道計算結果のファイルを作成
        ...
        if (iif .eq. 1) then
        write (filename, '("./orb/orb", i4.4, "bbr.txt")') ipm; !      write(6,*) filename
        write (filenameJ, '("./orb/jp", i4.4, "bbr.txt")') ipm; !      write(6,*) filename
        write (filenamemu, '("./orb/mu", i4.4, "bbr.txt")') ipm; !      write(6,*) filename
!        write (filenameP, '("./orb/p", i4.4, "bbr.txt")') ipm; !      write(6,*) filename
        else
        write (filename, '("orb", i4.4, "bbr.txt")') ipm; !      write(6,*) filename
        write (filenameJ, '("jp", i4.4, "bbr.txt")') ipm; !      write(6,*) filename
        write (filenamemu, '("mu", i4.4, "bbr.txt")') ipm; !      write(6,*) filename
!        write (filenameP, '("p", i4.4, "bbr.txt")') ipm; !      write(6,*) filename
        endif
...
!     ######################################################################################
!     save first, calculation ob beta etc. is next. 最初に記録，beta等を最後に計算する事にした．
!C    ##### B-B method pusher, start ####
!     relativistic case 1, according to Matsumoto
      do it = 0,iend+1

!C     time step for t^n,v^n-1/2 t^n,v^n-1/2の時間ステップを決める
!      if(its == 0) then ! constant time step
      Hr = dt
      Tr = dt*it;
...
!      gr1 = dsqrt( (x(1)-gr)*(x(1)-gr)+x(2)*x(2) ) ! use x^n x^n 使用
...
!     ループの終わりで v^n+1/2 について計算したので，ここではv^n-1/2の値
...
!     ## save data in a file ファイルに値を記録, start ##
      ...
      call bmap(elik2,elie2,x(1),x(2),x(3),Psi); Psi1=Psi; ! \Psi 磁気面関数の計算

!     guiding center calculation with real velocity 案内中心の座標を計算 real velocityで 
      Vtpp1 = ( u(1)-Bx/Btt*(u(1)*Bx+u(2)*By+u(3)*Bz)/Btt )/xgm; ! get Vperp 磁場成分のpara成分を引く->Vperp
...
!C     ########## J ##########
!C     integration along B, data out put at every z=0
!     Jを計算するために，磁力線に沿った積分を行う．赤道面に帰るたびに出力．
...
!C    T vector
        do i = 1,3
        Tv(i) = eom*B(i)*Hr*0.5d0/xgm ! this \gamma should be ^0, not ^-1/2 この \gammaは^-1/2でなく^0であるべき
...
!C     四元速度 u(1),u(2),u(3)から実速度やbeta gammaを出す．v^2=c^2 u^2 /(c^2+u^2),: ## ^n+1/2 ##
!C    'XU (four velocity), XV (real velocity)'
...
!      print "(a5,1p3d12.4,a10,1p2d12.4)","3,x",stx(3,1),stx(3,2),stx(3,3),"ave/sig:",stxa(3),stxs(3)
      ...
      open (51,File="orb_a0bbr.txt") ! save file 0, average 記録ファイル
      do isti=1,imbk2t
...
!     1:x 2:y 3:z 4:r 5:gx 6:gy 7:gz 8:gr 9:K 10:Kpr 11:Kpp 12:Ps 13:gPs 14:mu 15:time
      ...
      open (52,File="orb_a1bbr.txt") ! save file 1, average+sigma 記録ファイル
      do isti=1,imbk2t
      ...
      open (53,File="orb_a2bbr.txt") ! save file 2, average-sigma 記録ファイル
      do isti=1,imbk2t
...
!     これが使われるのは，RKで初期値のn^-1/2を計算する時だけ．
!    ***** 時間逆進用にマイナス符号付き *****
!C ###################################################
      use mod_variable
      implicit none
...
!C     四元速度 x(4),x(5),x(6)から実速度を出す．v^2=c^2 u^2 /(c^2+u^2)
      xu = dsqrt(x(4)*x(4)+x(5)*x(5)+x(6)*x(6))
      xv = XC*xu/dsqrt(xc*xc+xu*xu)
!C      XV=XU/dsqrt(1.0+(XU/XC)*(XU/XC)) <-larger error こちらの方が誤差が大きい
...
!C     return B at (x,y,z) obtained by interpolation xyz座標の点で \psi=2*pi*r*Athetaを計算して返す
!C     =================================================================
      use mod_variable
      implicit none
