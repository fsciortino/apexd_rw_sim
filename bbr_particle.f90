...
!    =================================================================
      ...
      integer :: nx,ny,nz,n0x,n0y,n0z,iexb,iele,ielemax,icln
...
!     =================================================================
      subroutine bbr_sim(pmx,pmy,pmz,pmk1,pmk2,pma,pmrwa,pmrwf,pmrwfg,pmrwd, &
           ipmt,dt,iend,imbk,imbk2,tE1,ci1,rc1,pid,iexe,mm,Evers)  
!    
!     This version was modified by FS in order to compile into a Python shared library.
!
!     Compile using a signature file:
!        
!        f2py bbr_particle.f90 -m apexd_rw_sim -h bbr_particle.pyf only: bbr_sim --overwrite-signature
!        --- modify .pyf file here --- (use given template and refer to instructions in F.Sciortino's project report)
...
!           
!        Note that it is possible to modify this code to have data directly given as subroutine outputs (i.e. kept in memory)
!        but this might run into a memory limit for long/complex simulations.
!        
!    =================================================================
      use mod_variable
      implicit none
...
!f2py intent(in) :: imbk, imbk2
      ...
      integer :: ikene,ipm,ipm1,ipm2  !, its 
...
!f2py intent(in) :: pmrwa,pmrwf,pmrwfg,pmrwd      
      ...
      double precision, dimension(1:105,1:10005) :: stK,stKpr,stKpp,stps1,stps2,stmu,strt !dimension(time step, particle#)
...
!     B is given by subroutine calculation or linear interpolation of external file
      iexb=0; ! 0: Biot-Savart calculation, 1: use external B field file
...
!     Pre-set voltages and frequencies of electrodes. (currently redundant, but useful for future upgrades) 
      ...
      if (iexb == 1) then !   B files to be used with interpolation. Not recommended. 
...
!     If header ('hd') file(s) are not read, xmax, xmin, etc. are still to be chosen at this point
...
!     ##### initial values of E and B at x^0 ####
      ...
      if (iexb == 1) then ! B is given by interpolation of external file data
         call bvac(elik2,elie2,x(1),x(2),x(3),Bx,By,Bz);
         print "(1p3d13.5,a35)",B(1),B(2),B(3),": B (T) with usual calculation"
...
!      print "(1p1d10.2,a2,1p3d10.2,a3,1p3d10.2)",tini," ",xr(1),xr(2),xr(3)," ",xr(4),xr(5),xr(6)
      open (2,file="backr.txt")
        ...
        Tr = tini + it*ddt      
        call rk4(elik2,elie2,Nr,Tr,Xr,K1,K2,K3,K4,YWORK,Hr,iexe,tE1,mm)
...
!      istep/imbk2+1 gives the total expected number of time steps (if particle doesn't hit a physical boundary)
!      ipmt = ipm2-ipm1+1 gives total particle number, or maximum value of ipm 
...
!     ############################################################
      ...
      if (iexb == 1) then !    B interpolation
         CALL bvac2(x(1),x(2),x(3),Bx,By,Bz)
      endif
      B(1)=Bx; B(2)=By; B(3)=Bz;
...
!      endif
      ...
      end do ! comes from do ipm=ipm1,ipm2      
...
!  ###################################################
      ...
      do 10 i=1,Nr
  10  k1(i)=Hr*F(elik2,elie2,i,Tr,Xr,iexe,tE1,mm)
      do 11 i=1,Nr
  11  ywork(i)=Xr(i)+0.5d0*k1(i)
      do 12 i=1,Nr
  12  k2(i)=Hr*F(elik2,elie2,i,Tr+0.5d0*Hr,ywork,iexe,tE1,mm)
      do 13 i=1,Nr
  13  YWORK(i)=Xr(i)+0.5d0*k2(i)
      do 14 i=1,Nr
  14  k3(i)=Hr*F(elik2,elie2,i,Tr+0.5d0*Hr,ywork,iexe,tE1,mm)
      do 15 i=1,Nr
  15  ywork(i)=Xr(i)+K3(i)
      do 16 i=1,Nr
  16  k4(i)=Hr*F(elik2,elie2,i,Tr+Hr,ywork,iexe,tE1,mm)
      do 17 i=1,Nr
...
!  ###################################################
      function F(elik2,elie2,i,t,x,iexe,tE1,mm)
...
!      KENE=XME*XC*XC*(XGM-1.0)
      ...
      if (iexb == 1) then !    interpolate B in external files 
